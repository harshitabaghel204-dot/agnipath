...
// 1. SVG Timeline Path (Desktop only)
const TimelinePath = ({ progress, containerHeight, activeSection }) => {
  const pathRef = useRef(null);
  const [pathLength, setPathLength] = useState(0);
  const [chipPosition, setChipPosition] = useState({ x: 0, y: 0, angle: 0 });
...
// 2. Mobile Timeline - Vertical Line with Dot and Chip
const MobileTimeline = ({ activeSection, totalSections, scrollProgress }) => {
  const progressPercent = ((activeSection + 1) / totalSections) * 100;
  const chipImageIndex = activeSection % chipImages.length;
  
  return (
    <>
      {/* Vertical Line */}
      <div className="md:hidden absolute left-8 top-0 bottom-0 w-0.5 bg-white/20 z-[5]">
        {/* Progress line */}
        <div 
          className="absolute top-0 left-0 w-full bg-gradient-to-b from-[#c8a357] to-[#c8a357]/50 transition-all duration-500"
        ...
        {journey.map((_, i) => (
          <div
            key={i}
            className="absolute left-1/2 -translate-x-1/2 transition-all duration-500 z-[15]"
            style={{ top: `${(i / (totalSections - 1)) * 100}%` }}
          >
            <div
              className={`w-3 h-3 rounded-full border-2 transition-all duration-500 ${
                i <= activeSection
                  ? 'bg-[#c8a357] border-[#c8a357] scale-125'
                  : 'bg-[#010317] border-white/30'
              }`}
            >
              {i === activeSection && (
                <div className="absolute inset-0 rounded-full bg-[#c8a357] animate-ping opacity-50" />
              )}
            </div>
          </div>
        ))}
      ...
      {/* Moving Chip Packet - OUTSIDE the line container for higher z-index */}
      <div 
        className="md:hidden absolute left-8 transition-all duration-300 ease-out pointer-events-none z-[20]"
        style={{ 
      ...
      >
        <div className="relative -translate-x-1/2 -translate-y-1/2">
          {/* Glow effect */}
          <div className="absolute inset-0 bg-[#c8a357] rounded-full blur-xl opacity-40 scale-150 -z-10" />
          
          {/* Chip Image */}
          <img
            src={chipImages[chipImageIndex]}
            alt="Balaji Chips"
            className="relative w-14 h-[70px] object-contain drop-shadow-2xl transition-all duration-300"
            style={{
              filter: 'drop-shadow(0 10px 25px rgba(200, 163, 87, 0.5))'
            }}
          />
        </div>
      </div>
...
// 3. Journey Section — Mobile Optimized
const JourneySection = ({ item, index, isActive, sectionRef }) => {
  const isEven = index % 2 === 0;
  const firstItemSpacing = index === 0 ? 'mt-12 md:mt-48' : '';

  return (
    <div
      ref={sectionRef}
      className={`relative z-10 ${firstItemSpacing}`}
    >
      {/* MOBILE LAYOUT */}
      <div className="md:hidden pl-16 pr-4 py-8">
        {/* Year Badge - Mobile */}
        <div
          className={`inline-block bg-[#c8a357] text-[#010317] px-4 py-1.5 rounded-full mb-4 transition-all duration-500 ${
            isActive ? 'scale-100 opacity-100' : 'scale-90 opacity-50'
          }`}
        >
          <span className="font-serif font-bold italic text-sm">{item.year}</span>
        </div>

        {/* Title - Mobile */}
        <h4
          className={`font-sans text-lg font-bold uppercase tracking-wider mb-3 transition-all duration-500 ${
        ...
        <div className="relative mb-4">
          <div
            className={`absolute inset-0 rounded-xl transition-all duration-500 ${
              isActive ? 'bg-[#c8a357] blur-lg opacity-20' : 'opacity-0'
            }`}
          />
          <img
            src={item.image}
            alt={item.title}
            className={`relative z-10 rounded-xl w-full h-48 object-cover transition-all duration-500 ${
              isActive ? 'shadow-lg shadow-[#c8a357]/30' : 'opacity-60'
            }`}
            loading="lazy"
          />
        </div>
        ...
        {/* Text - Mobile */}
        <p
          className={`font-sans text-base leading-7 font-light transition-all duration-500 ${
            isActive ? 'text-slate-300' : 'text-slate-500'
          }`}
        >
          {item.text}
        ...
        <div className="mt-6 flex items-center gap-2">
          <div
            className={`h-0.5 rounded-full transition-all duration-700 ${
              isActive ? 'w-16 bg-[#c8a357]' : 'w-8 bg-white/20'
            }`}
          />
          <div
            className={`h-0.5 rounded-full transition-all duration-700 delay-100 ${
              isActive ? 'w-6 bg-[#c8a357]/60' : 'w-3 bg-white/10'
            }`}
          />
        </div>
      </div>
      ...
      {/* DESKTOP LAYOUT */}
      <div
        className={`hidden md:flex items-center gap-8 md:gap-16 py-20 md:py-32 ${
          isEven ? 'md:flex-row' : 'md:flex-row-reverse'
        }`}
      >
        <div
...
// 4. Side Progress Dots (Desktop only)
const ProgressIndicator = ({ activeSection, onSectionClick }) => (
  <div className="fixed left-4 top-1/2 -translate-y-1/2 z-40 hidden lg:flex flex-col gap-2">
    {journey.map((item, i) => (
      <button
    ...
    const handleScroll = () => {
      ...
      for (let i = sectionRefs.current.length - 1; i >= 0; i--) {
        ...
        if (section) {
          ...
          if (viewportMiddle >= sectionMiddle - 100) {
            setActiveSection(i);
            break;
          }
        }
